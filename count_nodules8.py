...
def neigh(label,i,j,set_eq):
	...
	if j == 0 and i == 0:
		neighbour = [label[i+1][j], label[i][j+1], label[i+1][j+1]]
		...
		neighbour = [label[i-1][j], label[i+1][j], label[i][j+1], label[i-1][j+1], label[i+1][j+1]]
	elif j == len(label[i])-1 and i != 0 and i != len(label)-1:
		neighbour = [label[i-1][j], label[i+1][j], label[i][j-1], label[i-1][j-1], label[i+1][j-1]]
	elif j != 0  and j != len(label[i])-1 and i == 0:
		neighbour = [label[i+1][j], label[i][j+1], label[i][j-1], label[i+1][j-1], label[i+1][j+1]]
	elif j != 0  and j != len(label[i])-1 and i == len(label)-1:
		neighbour = [label[i-1][j], label[i][j+1], label[i][j-1], label[i-1][j-1], label[i-1][j+1]]
	elif j != 0 and i != 0 and j != len(label[i])-1 and i != len(label)-1:
		neighbour = [label[i-1][j], label[i+1][j], label[i][j+1], label[i][j-1], label[i-1][j-1], label[i-1][j+1], label[i+1][j-1], label[i+1][j+1]]
